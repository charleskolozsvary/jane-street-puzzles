 ...
 */
void extendLines(long numOfSplits, long width, long height, long startRowIdx, int spacer, struct linkedList * wll, FILE * fptr){
    long innerWidth = 1;
    for (long i = 0; i < height-1; i++){
        nSpaces(2, fptr);
        for (long j = 0; j < numOfSplits; j++){
	    nSpaces(width-1, fptr);
	    inLL(startRowIdx + j*2, wll) ? fputc('/', fptr) : fputc(' ', fptr);
            nSpaces(innerWidth, fptr);
            inLL(startRowIdx + j*2 + 1, wll) ? fputc('\\', fptr) : fputc(' ', fptr);
            nSpaces(width-1, fptr);
	    nSpaces(spacer, fptr);
        }
        fputc('\n', fptr);
...
void printNodes(long numNodes, long width, struct node * nodePres, int level, int spacer, FILE * fptr){
    fprintf(fptr, "%i ", level+1);
    for (long i = 0; i < numNodes; i++){
        nSpaces(width, fptr);
        nodePres[i].val ? fputc('A', fptr) : fputc('B', fptr);
        nSpaces(width, fptr);
        nSpaces(spacer, fptr);
    }
    fputc('\n', fptr);
...
void printBTree(int depth, struct node * treeArr, struct linkedList * wll, int doShort, FILE * fptr){
    int spacer = 3;
    ...
    for (int i = 0; i < depth; i++){
        ...
        if (!doShort)
	    extendLines((long)pow(2, i), (long)pow(2, depth+1-i)-2, height, startRowIdx, spacer, wll, fptr);
        printNodes((long)pow(2, i+1), width-2, &treeArr[startRowIdx], i, spacer, fptr);
        width /= 2; height /= 2;
    }
}
...
long bruteForceWinningOrientations(int depth){
    ...
    printf("\ndepth = %i\n", depth);
    printf("%-15s: %li/%ld\n%-15s: %3.6f\n", "winning/total", winCount, (long)pow(2, numEdges), "percentage", (double)winCount/pow(2, numEdges));
    return winCount;
}
...
void compareSimulatedAndRecurrenceAndExplicitProb(double p, long simulateIter, int maxDepth){
    printf("\nSimulated Probabilities (Iterations = %li)\n", simulateIter);
...
void simulateAndPrintWinningTrees(double p, long simulateIter, int maxDepth, FILE * fptr){
        double runningAvg = 0;
        int count = 0;
        for (int i = 0; i < simulateIter; i++){
            ...
            if (AaronWins){
                ...
                count++;
                printBTree(maxDepth, tree, cas, 0, fptr);
	...
	printf("Number of winning trees = %i\n", count);
        printf("Average number of nodes in winning casacde = %3.5f\n", runningAvg);
}

/*
  Usage: gcc triage.c   (or also -o [name of executable])  
         ./a.out        (or ./[name of executable])
 */
int main(int argc, char** argv){
    initRandom();
    
    /*
     Compare simulated, calculated recurrence, and expanded exact probabilities
     (the latter two from formulas) for finite depth and p = p1.
     */
    double p1 = .5;
    long simulateIter1 = 1000000;
    int maxDepth1 = 7;
    printf("COMPARING SIMULATED and EXACT PROBABILITIES");
    compareSimulatedAndRecurrenceAndExplicitProb(p1, simulateIter1, maxDepth1);
    printf("\n\n");
     ...
     */
    printf("BRUTE FORCE PROBABILITIES:");
    ...
    long cutoffIter = 10000000000;
    long checkIter  = 1000000000;
    printf("STEADY STATE CHECK\n");
    printf("p = %.20f\n", p2);
    seeifSteadies(p2, cutoffIter, checkIter);
    printf("\n\n");
     ...
     */
    double p3 = .9;
    long simulateIter2 = 50;
    int maxDepth2 = 7;

    FILE * fptr;
    fptr = fopen("tree-drawings.txt", "w");
    
    printf("ANOTHER SIMULATION (DRAWING TREES TO FILE)\n");
    
    simulateAndPrintWinningTrees(p3, simulateIter2, maxDepth2, fptr);
    fclose(fptr);
    printf("Winning trees were written to tree-drawing.txt\n");
